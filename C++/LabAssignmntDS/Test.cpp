...
cout<<"Date: "<<tollEntry.date<<" Ticket Number: "<}
}
...
//cout<<"Entry: "<tollBox1.addEntry(tollInfo);
ticketNum++;
}
cout<<"Tax collected by Box1 "<for(auto vehicle: vehicleList2){
time_t now = time(0);
char* current_date = ctime(&now);
TollInformation tollInfo = {.carNumber = vehicle.getVehicleNumber(),.date = current_date, .taxAmount = TollSystem::getTollTax(vehicle.getVehicleType()),.ticketNumber =ticketNum};
//cout<<"Entry: "<tollBox2.addEntry(tollInfo);
ticketNum++;
}
cout<<"Tax collected by Box2 "<for(auto vehicle: vehicleList3){
time_t now = time(0);
char* current_date = ctime(&now);
TollInformation tollInfo = {.carNumber = vehicle.getVehicleNumber(),.date = current_date, .taxAmount = TollSystem::getTollTax(vehicle.getVehicleType()),.ticketNumber =ticketNum};
